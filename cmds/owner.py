...
class Owner(ExtensionBase):
	...
	@commands.command()
	async def respond_command_test(self, ctx):
		await ctx.send("Respond_command_sucessful!")

	@commands.command()
	async def full_test(self, ctx):
		time.sleep(5)
		await ctx.send("Build Sucess!. Used time 5.36")
		time.sleep(4)
		await ctx.send("Respond Sucess. Used time 4.21")
		time.sleep(`8`)
